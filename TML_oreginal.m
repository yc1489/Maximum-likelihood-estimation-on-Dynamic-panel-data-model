...
sig2_u_ini = sum(sum(du.*du))/(2*N*(T-2)); % 3.9
...
if MDE_phi(2,1) > 1; MDE_phi(2,1) = 0.999; end
   ...
   theta_ini = [rand(2,1);  rand(1,1);  rand(1,1)];

    lq = [ -inf;   -inf;    (T-1)/T;   0.0001 ];
    uq = [ inf;     inf;       inf;     inf  ];
   

[theta, fval, exitflag, output, lambda, hessian] = TMLopt(N,T,Dy,DW,Phi,Omega,theta_ini,lq,uq);
